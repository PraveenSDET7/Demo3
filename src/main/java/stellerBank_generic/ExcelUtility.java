...
public class ExcelUtility 
{
	...
	public void insert_Data_Into_Excel_Increamentelly(String SheetName,String value) throws EncryptedDocumentException, IOException
	{
		FileInputStream fileInputStream = new FileInputStream(PathConstants.excelfilepath);
		Workbook book = WorkbookFactory.create(fileInputStream);
		Sheet sheet = book.getSheet(SheetName);
		...
		{
			int cell=sheet.getRow(RowNo-1).getPhysicalNumberOfCells();
			if(CellNo<cell)
			{
				sheet.createRow(RowNo).createCell(cell-1);
			}
			else
			{
				sheet.createRow(RowNo+1).createCell(cell-1);
			}
		}
		FileOutputStream fileOutputStream = new FileOutputStream(PathConstants.excelfilepath);
		book.write(fileOutputStream);
